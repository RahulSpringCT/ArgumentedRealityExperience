...
//server = "ws://172.24.12.156:8188/janus";
...
$(document).ready(function () {
	...
	Janus.init({
		debug: true,
		iceServers: [{ "host": "turn-beta.loookit.com", "port": "3478", "username": "loookit", "password": "eTm7rb3WBYGx6P" }],
		callback: function () {
							...
							janus.attach(
								{
									...
									onlocalstream: function (stream) {
										Janus.debug(" ::: Got a local stream :::", stream, amIcaller);
										$('#videos').removeClass('hide').show();
										if ($('#myvideo').length === 0 && !amIcaller)
										...
										if (amIcaller) {
											console.log('showing remote facial');
											$('#remoteFacial').removeClass('hide').show(); // here i am tried for conditionally use of dataelement.
										}
									},
	...
	videocall.createOffer(
		{
			...
			// stream: canvasMediaStream(),
			stream: engine.getRenderingCanvas().captureStream(25),
...
// function defined for capture the media stream from canvas.
function canvasMediaStream() {

	// vid = document.getElementById("vid");
	// mediaStream = vid.captureStream();
	...
	// return await recorder.startRecording(null, 1).then((videoBlob) => {
	// 	// Do Something with the videoBlob.
	// 	console.log('videoBlob:', videoBlob);
	// 	vid.src = URL.createObjectURL(videoBlob); 
	let mediaStream = vid.captureStream(25);
	return mediaStream;
	// });
	// }


	mediaStream = vid.captureStream(10);
	// vid.srcObject = mediaStream;

	console.log("MediaStream from canvas : ", mediaStream);
	return mediaStream;
}
